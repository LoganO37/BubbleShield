  ...
  try {
    ...
    const mergeList = (listId, defaultList) => {
      ...
      if (added && removed) {
        ...
        // Normalize once and create sets
        const defaultNormalized = defaultList.map(k => normalizeText(k));
        const removedNormalized = removed.map(k => normalizeText(k));
        const defaultSet = new Set(defaultNormalized);
        const removedSet = new Set(removedNormalized);

        const filteredDefault = defaultList.filter((k, i) => !removedSet.has(defaultNormalized[i]));
        ...
        if (savedList) {
          ...
          // Normalize once per list
          const defaultNormalized = defaultList.map(k => normalizeText(k));
          const savedNormalized = savedList.map(k => normalizeText(k));
          const defaultSet = new Set(defaultNormalized);
          const savedSet = new Set(savedNormalized);

          // Find words in savedList that are NOT in defaultList (User added)
          const userAdded = savedList.filter((k, i) => !defaultSet.has(savedNormalized[i]));
...
function handleListChange() {
  ...
  // Normalize once, then deduplicate
  const normalized = keywords.map(k => normalizeText(k)).filter(k => k.length > 0);
  loadedLists[currentListId] = [...new Set(normalized)];
...
async function saveKeywords() {
  try {
    ...
    // Normalize once, then deduplicate
    const normalized = currentKeywords.map(k => normalizeText(k)).filter(k => k.length > 0);
    loadedLists[currentListId] = [...new Set(normalized)];
    ...
    const calculateDiff = (listId, currentList, defaultList) => {
      // Normalize once per list
      const currentNormalized = currentList.map(k => normalizeText(k));
      const defaultNormalized = defaultList.map(k => normalizeText(k));
      const currentSet = new Set(currentNormalized);
      const defaultSet = new Set(defaultNormalized);

      // Added = Current - Default
      const added = currentList.filter((k, i) => !defaultSet.has(currentNormalized[i]));

      // Removed = Default - Current
      const removed = defaultList.filter((k, i) => !currentSet.has(defaultNormalized[i]));
    ...
    // Deduplicate final list (keywords should already be normalized from loadedLists)
    const uniqueAllKeywords = [...new Set(allKeywords)].filter(k => k.length > 0);
