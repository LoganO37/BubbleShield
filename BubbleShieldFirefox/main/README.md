...
- ✅ **Smart caching** - Results cached for 48 hours to minimize repeated checks
...
6. Results are cached for 48 hours to avoid re-checking the same users
...
```
BubbleShield/
├── manifest.json       # Extension configuration (Manifest V3 for Firefox)
├── manifest-firefox.json  # Firefox-specific manifest backup
├── background.js       # Background script for extension lifecycle
├── content_script.js   # Content script for tweet detection and hiding
├── options.html        # Options page UI
├── options.js          # Options page logic
├── lists.js            # Premade keyword lists
├── BubbleShield128.png # Extension icon
└── README.md          # This file
```
...
### `manifest.json`
Defines the extension structure, permissions, and configuration. Uses Manifest V3 for Firefox 109+.

### `background.js` (Background Script)
- Minimal background script for extension lifecycle management
- Sets default blocklist on first install
...
- Caches bio data for 48 hours to minimize hover card triggers

### `options.html` + `options.js`
- Provides a clean UI for managing blocked keywords
- Saves keywords to browser storage (uses cross-browser compatible API)
...
### Performance issues
- The extension caches results for 48 hours
- If you notice slowdowns, try reducing the number of blocked keywords
- Each unique user is only checked once per 48 hours
...
### Debugging
- **Background script**: Go to `about:debugging` → Click "Inspect" next to Bubble Shield
